...
| **Verification** | `SOURCIFY`, `ETHERSCAN`, `BLOCKSCOUT`, or `Unverified` |
...
<Note>
  `BYTECODE-DB` doesn't appear here. Hexkit can resolve an ABI from a bytecode
  database, but that only happens when you load a contract directly — see
  [Call a contract](/guides/call-a-contract). Simulation results resolve verification
  from Sourcify, Etherscan, or Blockscout only.
</Note>
